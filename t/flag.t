...
tap:run [
  [&d='parse with remote and positional args' &f={
    ...
    tap:assert-expected $opts [&remote=myremote &help=$false &dry-run=$false &keep-count=1 &prefix=backup &junk-paths=$false &index=0]
  ...
  [&d='parse with environment RBACKUP_KEEP_COUNT' &f={
    ...
    var opts sub paths = (flag:parse -r myremote generate path/to/backup)
    tap:assert-expected $opts [&remote=myremote &help=$false &dry-run=$false &keep-count=2 &prefix=backup &junk-paths=$false &index=0]
    tap:assert-expected $sub 'generate'
    tap:assert-expected $paths [path/to/backup]
    unset-env RBACKUP_KEEP_COUNT
  }]
  [&d='parse with --help flag' &f={
    ...
    tap:assert-expected $opts [&remote='' &help=$true &dry-run=$false &keep-count=1 &prefix=backup &junk-paths=$false &index=0]
    tap:assert-expected $sub 'generate'
    tap:assert-expected $paths []
  }]
  [&d='parse with -d flag' &f={
    ...
    tap:assert-expected $opts [&remote='' &help=$false &dry-run=$true &keep-count=1 &prefix=backup &junk-paths=$false &index=0]
  ...
  [&d='parse with environment RBACKUP_FILE_PREFIX' &f={
    ...
    var opts sub paths = (flag:parse -r myremote generate path/to/backup)
    tap:assert-expected $opts [&remote=myremote &help=$false &dry-run=$false &keep-count=1 &prefix=custom-prefix &junk-paths=$false &index=0]
    tap:assert-expected $sub 'generate'
    tap:assert-expected $paths [path/to/backup]
    unset-env RBACKUP_FILE_PREFIX
  }]
  [&d='parse with -p flag' &f={
    ...
    tap:assert-expected $opts [&remote='' &help=$false &dry-run=$false &keep-count=1 &prefix=test-prefix &junk-paths=$false &index=0]
    tap:assert-expected $sub 'generate'
    tap:assert-expected $paths []
  }]
  ...
  [&d='parse with -j flag' &f={
    ...
    tap:assert-expected $opts [&remote='' &help=$false &dry-run=$false &keep-count=1 &prefix=backup &junk-paths=$true &index=0]
    tap:assert-expected $sub 'generate'
    tap:assert-expected $paths []
  }]
  ...
  [&d='parse with -i flag' &f={
    ...
    tap:assert-expected $opts [&remote='' &help=$false &dry-run=$false &keep-count=1 &prefix=backup &junk-paths=$false &index='2']
