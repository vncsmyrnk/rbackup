...
tap:run [
  [&d='parse with remote and positional args' &f={
    ...
    tap:assert-expected $opts [&remote=myremote &help=$false &dry-run=$false &keep-count=1 &prefix=backup]
  ...
  [&d='parse with environment RBACKUP_RCLONE_REMOTE' &f={
    ...
    tap:assert-expected $opts [&remote=envremote &help=$false &dry-run=$false &keep-count=1 &prefix=backup]
    tap:assert-expected $sub 'generate'
    tap:assert-expected $paths [path/to/backup]
  ...
  [&d='parse with positional args and RBACKUP_PATHS' &f={
    ...
    tap:assert-expected $opts [&remote='' &help=$false &dry-run=$false &keep-count=1 &prefix=backup]
    tap:assert-expected $sub "generate"
    tap:assert-expected $paths [path1 path2]
    unset-env RBACKUP_PATHS
  }]
  [&d='parse with environment RBACKUP_KEEP_COUNT' &f={
    ...
    var opts sub paths = (flag:parse -r myremote generate path/to/backup)
    tap:assert-expected $opts [&remote=myremote &help=$false &dry-run=$false &keep-count=2 &prefix=backup]
    tap:assert-expected $sub 'generate'
    tap:assert-expected $paths [path/to/backup]
  ...
  [&d='parse with --help flag' &f={
    ...
    tap:assert-expected $opts [&remote='' &help=$true &dry-run=$false &keep-count=1 &prefix=backup]
    tap:assert-expected $sub 'generate'
    tap:assert-expected $paths []
  }]
  [&d='parse with -d flag' &f={
    var opts sub paths = (flag:parse -d gc)
    tap:assert-expected $opts [&remote='' &help=$false &dry-run=$true &keep-count=1 &prefix=backup]
  ...
  [&d='parse with environment RBACKUP_FILE_PREFIX' &f={
    ...
    var opts sub paths = (flag:parse -r myremote generate path/to/backup)
    tap:assert-expected $opts [&remote=myremote &help=$false &dry-run=$false &keep-count=1 &prefix=custom-prefix]
    tap:assert-expected $sub 'generate'
    tap:assert-expected $paths [path/to/backup]
  ...
  [&d='parse with -p flag' &f={
    ...
    tap:assert-expected $opts [&remote='' &help=$false &dry-run=$false &keep-count=1 &prefix=test-prefix]
    tap:assert-expected $sub 'generate'
    tap:assert-expected $paths []
  }]
